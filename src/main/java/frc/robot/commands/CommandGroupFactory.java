...
public class CommandGroupFactory {
    ...
    public static Command collectUntilNote(){
        //return new SequentialCommandGroup(intake.coolectUntilNoteCommand(),intake.feedBack());
        return new ParallelDeadlineGroup(intake.setCurrentCommand()).andThen(Commands.waitUntil(() -> intake.getSwitchCommand())).andThen(intake.feedBack());
    }
